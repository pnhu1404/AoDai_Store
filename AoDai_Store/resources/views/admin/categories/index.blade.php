    ...
    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        <div class="flex flex-col md:flex-row justify-between gap-4">
            <form action="{{ route('admin.categories.index') }}" method="GET" class="flex flex-1 gap-2">
                ...
                @if(request('search'))
                    <a href="{{ route('admin.categories.index') }}" class="bg-gray-100 text-gray-600 px-4 py-2 rounded-lg flex items-center">
                        Xóa lọc
                    </a>
                @endif
            </form>

            <a href="{{ route('admin.categories.create') }}" class="inline-flex items-center justify-center bg-green-600 text-white px-6 py-2 rounded-lg hover:bg-green-700 transition font-semibold shadow-md">
                ...
                <tr class="hover:bg-indigo-50/30 transition duration-150">
                    <td class="px-6 py-4">
                        <div class="flex items-center">
                            <div class="w-10 h-10 bg-indigo-100 text-indigo-600 rounded-lg flex items-center justify-center mr-3 font-bold">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex justify-center space-x-3 text-sm">
                            <a href="{{ route('admin.categories.edit', $category->MaLoaiSP) }}" class="text-blue-600 hover:text-blue-800 transition p-2 bg-blue-50 rounded-full" title="Chỉnh sửa">
                            ...
                            <form id="delete-form-{{ $category->MaLoaiSP }}" action="{{ route('admin.categories.destroy', $category->MaLoaiSP) }}" method="POST" class="hidden">
                                @csrf @method('DELETE')
                            </form>
                        </div>
                    </td>
...
{{-- Giữ nguyên Script SweetAlert2 và Style của bạn để đồng bộ giao diện --}}
        ...
        Swal.fire({
            ...
            html: `
                <div class="mt-3 text-stone-500 text-sm">
                    Danh mục <b class="text-stone-900 italic">"${name}"</b> sẽ bị xóa. 
                    <br><span class="text-red-600">Lưu ý: Các sản phẩm thuộc danh mục này có thể bị ảnh hưởng!</span>
                </div>
            `,
            icon: 'question', 
            iconColor: '#991b1b',
...
<style>
    /* Sử dụng lại class Style bạn đã cung cấp cho đồng bộ */
    .glass-popup {
